...
export const CITIES: Record<City, { state: string; lat: number; lon: number }> = {
  ...
  "Bengaluru, India": { state: "karnataka", lat: 12.9716, lon: 77.5946 },
  "Mumbai, India": { state: "maharashtra", lat: 19.076, lon: 72.8777 },
};

export type ForecastPoint = { datetime: string; carbonIntensity: number };
export type GridData = { available: boolean; city: City; current?: { carbonIntensity: number; datetime: string; isEstimated: boolean }; forecast: ForecastPoint[]; updatedAt?: string; source: string; error?: string };

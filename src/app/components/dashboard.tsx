...
export function Dashboard() {
  ...
  return <div className="mobile-app">
    ...
    <main className="dashboard-content">
      ...
      <section className="timeline-row"><Link href="/rewards" className="streak-card"><h2>Current Streak</h2><div><strong>{streak || "—"}</strong><span>{streak ? "day streak" : "Start today"}</span></div><Icon name="fire" size={104}/></Link><Link href="/forecast" className="card next-window"><h2>Next Charging Window</h2><div><span className="round-icon"><Icon name="clock" size={18}/></span><p>{recommendedWindow ? <><strong>{formatHour(recommendedWindow.start.datetime)} – {formatHour(recommendedWindow.end.datetime)}</strong><span>{Math.round(recommendedWindow.average)} gCO₂/kWh average</span></> : <><strong>Not available</strong><span>Check back when forecast loads</span></>}</p></div></Link></section>

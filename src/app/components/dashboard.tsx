...
export function Dashboard() {
  ...
  function chooseLocation(next: City) { setLocation(next); setLocationOpen(false); }
  return <div className="mobile-app">
    ...
    <main className="dashboard-content">
      ...
      <section className="recommendation card"><div className="recommendation-head"><h2>Best time to charge</h2>{recommendedWindow && <span className="recommended"><Icon name="bolt" size={15}/> {data?.forecastIsTypical ? "Typical pattern" : "Recommended"}</span>}</div><div className="time-range">{loading ? <strong className="loading-value">Loading...</strong> : recommendedWindow ? <><strong>{formatHour(recommendedWindow.start.datetime)}</strong><span>–</span><strong>{formatHour(recommendedWindow.end.datetime)}</strong></> : <strong className="unavailable-value">Unavailable</strong>}</div><div className="intensity"><span className="co2">CO₂</span> Current Intensity: <strong>{current ? `${Math.round(current.carbonIntensity)} gCO₂/kWh` : "Unavailable"}</strong></div>{intensityDiff !== null && (intensityDiff === 0 ? <p className="personal-note">About the same as your usual charging window</p> : <p className="personal-note">{Math.abs(intensityDiff)} gCO₂/kWh {intensityDiff < 0 ? "cleaner" : "higher"} than your usual charging window</p>)}<Link className="forecast-button" href="/forecast">View live forecast <Icon name="arrow" size={18}/></Link></section>
      ...
      <section className="timeline-row"><Link href="/rewards" className="streak-card"><h2>Current Streak</h2><div><strong>{streak || "—"}</strong><span>{streak ? "day streak" : "Start today"}</span></div><Icon name="fire" size={104}/></Link><Link href="/forecast" className="card next-window"><h2>Next Charging Window</h2><div><span className="round-icon"><Icon name="clock" size={18}/></span><p>{recommendedWindow ? <><strong>{formatHour(recommendedWindow.start.datetime)} – {formatHour(recommendedWindow.end.datetime)}</strong><span>{Math.round(recommendedWindow.average)} gCO₂/kWh average{data?.forecastIsTypical ? " · typical" : ""}</span></> : typicalTime ? <><strong>Around {formatTimeOfDay(typicalTime)}</strong><span>Based on your charging history</span></> : <><strong>Not available</strong><span>Check back when forecast loads</span></>}</p></div></Link></section>

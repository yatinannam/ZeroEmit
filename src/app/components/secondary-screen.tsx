...
export function SecondaryScreen({ page }: { page: keyof typeof content }) {
  ...
  return <div className="mobile-app secondary-app"><header className="top-app-bar"><Link href="/" aria-label="Back to home"><Icon name="arrow-left"/></Link><strong>ZeroEmit</strong><span aria-hidden /></header><main className="secondary-content"><p className="eyebrow">{data.eyebrow}</p><h1>{data.title}</h1><p className="secondary-intro">{data.intro}</p><section className="secondary-cards">{cards.map(([label, value, detail]) => <article className="card secondary-card" key={label}><h2>{label}</h2><strong>{value}</strong><span>{detail}</span></article>)}</section><button className="forecast-button action-button" onClick={primaryAction}>{actionLabel} <span>→</span></button>{message && <p className="success-message" role="status">{message}</p>}<Link className="text-link" href="/">Back to dashboard</Link></main>{formOpen && <div className="modal-backdrop" role="presentation" onClick={() => setFormOpen(false)}>{page === "log" ? <form className="modal charge-form" onSubmit={submitCharge} onClick={(event) => event.stopPropagation()}><div className="modal-head"><h2>Log a charge</h2><button type="button" className="modal-close" aria-label="Close" onClick={() => setFormOpen(false)}><Icon name="close" size={18}/></button></div><label>Energy used (kWh)<input name="energy" type="number" min="0.1" step="0.1" defaultValue={typicalEnergy ? typicalEnergy.toFixed(1) : "18"} required /></label><label>When did you charge?<input name="time" type="time" defaultValue={typicalTime ?? "02:30"} required /></label><button className="forecast-button" type="submit">Save charge</button></form> : <form className="modal charge-form" onSubmit={submitProfile} onClick={(event) => event.stopPropagation()}><div className="modal-head"><h2>Edit profile</h2><button type="button" className="modal-close" aria-label="Close" onClick={() => setFormOpen(false)}><Icon name="close" size={18}/></button></div><label>Name<input name="name" type="text" defaultValue={profile.name} placeholder="Your name" maxLength={60} /></label><label>Vehicle<input name="vehicle" type="text" defaultValue={profile.vehicle} placeholder="e.g. Tata Nexon EV" maxLength={60} /></label><button className="forecast-button" type="submit">Save profile</button></form>}</div>}</div>;
}
